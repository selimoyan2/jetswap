...
import { getItemConditionLabel, getTradeMethodLabel, formatLocalizedDate } from '@/i18n/helpers'
...
export function SavedSearchesClient({ initialSearches, userName }: SavedSearchesClientProps) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrorMessage(data.error?.message || (language === 'tr' ? 'Arama silinirken bir hata oluştu.' : 'Failed to delete search.'))
      }
    } catch {
      setErrorMessage(language === 'tr' ? 'Bağlantı hatası oluştu.' : 'Network connection error.')
    } finally {
  ...
  const handleSaveRename = async (id: string) => {
    ...
    if (!trimmed || trimmed.length > 80) {
      setErrorMessage(language === 'tr' ? 'Arama adı 1-80 karakter arasında olmalıdır.' : 'Search name must be between 1 and 80 characters.')
      return
    }
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setErrorMessage(data.error?.message || (language === 'tr' ? 'Güncelleme başarısız oldu.' : 'Update failed.'))
      }
    } catch {
      setErrorMessage(language === 'tr' ? 'Bağlantı hatası oluştu.' : 'Network connection error.')
    } finally {
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        >
          ...
          <span>{t.savedSearches.createNewSearchHint}</span>
        </Link>
      </div>
                      ...
                      {isEditing ? (
                        <div className="flex items-center gap-2 flex-1">
                          ...
                          <button
                            ...
                            title={t.savedSearches.saveButton}
                          >
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {!s.query && !s.category && !s.city && !s.condition && !s.tradeMethod && (
                        <span className="text-[11px] text-zinc-400 font-medium italic">
                          {language === 'tr' ? 'Tüm İlanlar (Filtresiz)' : 'All Listings (Unfiltered)'}
                        </span>
                      )}
                    </div>
                  </div>
