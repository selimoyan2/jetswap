...
function ItemCardSummary({ item }: { item: OfferItemSummary }) {
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl bg-zinc-900/60 border border-zinc-800 hover:border-zinc-700 transition-colors">
      <div className="relative w-16 h-16 rounded-lg overflow-hidden bg-zinc-800 flex-shrink-0 border border-zinc-700/50">
        {imageSrc ? (
      ...
      <div className="flex-1 min-w-0">
        <Link
          href={`/items/${item.id}`}
          className="font-medium text-white hover:text-emerald-400 transition-colors line-clamp-1 text-sm"
        >
          {item.title}
        </Link>
...
export const OfferExchangeView: React.FC<OfferExchangeViewProps> = ({
  ...
  return (
    <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-6 p-6 rounded-2xl bg-zinc-950/70 border border-zinc-800">
      {/* Visual Exchange Badge in the Center for desktop */}
      <div className="hidden lg:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-emerald-500/10 border border-emerald-500/30 items-center justify-center text-emerald-400 z-10 shadow-lg shadow-black/50">
      ...
      <div className="space-y-3">
        <div className="border-b border-zinc-800/80 pb-2">
          ...
          <p className="text-xs text-zinc-500 mt-0.5">{leftSubtitle}</p>
        </div>
        ...
        <div className="space-y-2">
          ...
          {leftItems.length === 0 && (
            <p className="text-xs text-zinc-500 italic py-2">Eşya bilgisi bulunamadı.</p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="border-b border-zinc-800/80 pb-2">
          ...
          <p className="text-xs text-zinc-500 mt-0.5">{rightSubtitle}</p>
        </div>
